...
-- 「Arubaの人口は103000人です」
SELECT CONCAT(name, 'の人口は', population, '人です') AS result 
FROM countries;
...
SELECT LEFT(code, 1) AS SUBSTRING, name 
FROM countries;
...
SELECT MIN(surface_area) AS '最小の表面積'
FROM countries
WHERE continent = 'Asia';

-- 問24
-- アジア大陸の表面積の合計を表示してください。
SELECT SUM(surface_area) AS 'アジア大陸の 表面積の合計'
FROM countries
WHERE continent = 'Asia';
...
SELECT countries.name AS 国名, cities.name AS 市区町村, countrylanguages.language AS language
FROM countries
JOIN countrylanguages ON countries.code = countrylanguages.country_code
...
SELECT celebrities.name AS name, countries.name AS name
FROM celebrities
LEFT JOIN countries  celebrities.country_code = countries.code;

-- 問28
-- 全ての有名人の名前,国名、第一言語を出力してください。
SELECT celebrities.name AS name, countries.name AS name, countrylanguages.language AS language
FROM celebrities
...
SELECT countries.code AS country_code, MAX(celebrities.age) AS max_age, MIN(celebrities.age) AS min_age
FROM countries 
...
(SELECT '1991' AS '誕生年', COUNT(*) AS '人数' FROM celebrities WHERE YEAR(birth) = 1991)
UNION ALL
